...
function parseURLMessage(url: string): ScanResult {
  ...
  try {
    ...
    // Check for short URL pattern (/wallet/i/<token>) — generated by the in-app URL shortener
    if (/\/wallet\/i\/[a-z0-9]+$/i.test(urlObj.pathname)) {
      console.log('🔗 [QR Parser] Detected short invitation URL, passing through for resolution');
      return {
        rawData: url,
        messageType: 'oob-invitation',
        parsedData: { invitationUrl: url, isShortUrl: true },
        routeAction: {
          page: '/oob',
          queryParams: { invitation: url }
        }
