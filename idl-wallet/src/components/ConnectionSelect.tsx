...
export const ConnectionSelect: React.FC<{
    ...
    // Handle default selection in useEffect to avoid setState during render
    useEffect(() => {
        if (connections.length > 0 && !props.defaultValue) {
            onHandleChange(connections[0].receiver.toString());
        }
    }, [connections, props.defaultValue]);
    ...
    // Default selection handled in useEffect to avoid setState during render
        ...
        <select
            ...
            className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            {
                connections.map((connection, i) => {
                    return <option key={connection.receiver.toString()} value={connection.receiver.toString()}>{connection.name}</option>
                })
            }
        </select>
