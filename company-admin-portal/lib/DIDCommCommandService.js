...
class DIDCommCommandService {
  ...
  constructor(config) {
    ...
    this._pendingGrants = new Map(); // proofId → grant body (for wallet HTTP polling)
  ...
  /**
   * Consume a pending grant for the given proofId (one-time, deletes on read).
   * Used by the wallet via HTTP polling instead of DIDComm BasicMessage polling.
   * Returns null if no grant is ready yet.
   */
  consumeGrant(proofId) {
    const grant = this._pendingGrants.get(proofId) ?? null;
    if (grant) this._pendingGrants.delete(proofId);
    return grant;
  }
  ...
  async _handleProtocolMessage(connectionId, envelope) {
    ...
    if (type === 'https://identuslabel.cz/protocols/access-request/1.0/request') {
      await this._handleAccessRequest(connectionId, body.target, requestId);
      return;
    }
  ...
  async _handleAccessRequest(connectionId, targetKey, requestId) {
    ...
    this._pending.set(proofId, { connectionId, targetKey, challenge, domain, requestId, startedAt: Date.now() });
  ...
  async _onProofSuccess(proofId, pending, presentationData) {
    ...
    // Store grant for wallet HTTP polling (wallet polls /api/enterprise-portal/grant-status?requestId=)
    // Key by requestId (from the access-request envelope id) — shared between wallet and server.
    // Proof IDs differ per-agent so requestId is the only stable shared key.
    ...
    if (pending.requestId) this._pendingGrants.set(pending.requestId, grantBody);
    this._pendingGrants.set(proofId, grantBody); // fallback: key by proofId too
   ...
   * and whose optional sub-objects carry data from CISTraining / SecurityClearanceGrant:
   *
